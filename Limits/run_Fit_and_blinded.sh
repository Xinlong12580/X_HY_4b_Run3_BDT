...
# Now we can run blinded expected limits using the Asimov dataset. Toys can be generated using the b-only fit result.
# Following instructions here: https://cms-analysis.github.io/HiggsAnalysis-CombinedLimit/part3/commonstatsmethods/?h=blind#asymptotic-frequentist-limits
seed=123456
if [ $limits = 1 ]; then 
    printf "cd $fitDir \n"
    cd $fitDir
    ...
    (set -x; combine -M AsymptoticLimits -m 125 -n "" -d higgsCombineSnapshot.MultiDimFit.mH125.root --snapshotName MultiDimFit -v $verbosity --saveWorkspace --saveToys --bypassFrequentistFit -s $seed --toysFrequentist --run blind)
