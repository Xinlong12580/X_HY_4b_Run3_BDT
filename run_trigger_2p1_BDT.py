...
columns = [ "Tagger_.*", "HLT_.*", "TFlavor.*", "MassHiggs.*", "idxJ.*", "MY", "MX", "leadingFatJetPt","leadingFatJetPhi","leadingFatJetEta", "leadingFatJetMsoftdrop", "PtJY0", "PtJY1", "EtaJY0", "EtaJY1", "PhiJY0", "PhiJY1", "MassJY0", "MassJY1", "MassJJH", "MassHiggsCandidate", "PtHiggsCandidate", "EtaHiggsCandidate", "PhiHiggsCandidate", "MassYCandidate", "MJJH", "MJY",  "Pileup_nTrueInt", "weight.*"]
...
for Reg in ["Signal"]:
    ...
    ana.selection_2p1_BDT(args.JME_syst, Reg, triggerType = "Reference")
    #ana.trainer_preprosessing_2p1()
    #Saving snapshot and cutflow
    file_basename = os.path.basename(args.dataset).removesuffix(".txt")
    ana.output = "Reg" + Reg[0:3] + "_" + args.JME_syst + "_2p1_trigger_tagged_selected_" + file_basename + f"_n-{args.n_files}_i-{args.i_job}.root"
