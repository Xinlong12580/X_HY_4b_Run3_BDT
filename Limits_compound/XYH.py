...
def _select_signal(row, args):
    ...
    if row.process_type == 'SIGNAL':    # This entry in the ledger (row) is of type "SIGNAL"
        ...
        #if row.process.startswith('QCD_fail'):
        if "SB1" in row.process:
...
def make_2DAlphabet_workspace(name='test', fr={}, json='XYH.json', r_fail = "VB1", r_pass = "VS2", PFR_MC = "1"):
    _rc_options = {
        '0x0': {
            'form': PFR_MC + '*(@0)',
        ...
        '1x1': {
            'form': PFR_MC + '*(@0+@1*x+@2*y+@3*x*y)',
            'constraints': _generate_constraints(4)
        },
        '2x0': {
            'form': PFR_MC + '*(@0+@1*x+@2*x*x)',
            'constraints': _generate_constraints(3)
        },
        '0x2': {
            'form': PFR_MC + '*(@0+@1*y+@2*y*y)',
            'constraints': _generate_constraints(3)
        },
        '2x1': {
            'form': PFR_MC + '*(@0+@1*x+@2*y+@3*x*y+@4*x**2+@5*y*x**2)',
            'constraints': _generate_constraints(5)
        },
        '1x2': {
            'form': PFR_MC + '*(@0+@1*x+@2*y+@3*x*y+@4*y**2+@5*x*y**2)',
            'constraints': _generate_constraints(5)
        },
        '2x2': {
            'form': PFR_MC + '* (@0+@1*x+@2*x**2)*(@3+@4*y*@5*y**2)',
    ...
    qcd_fail = BinnedDistribution(
        f'QCD_{r_fail}',         # Name of the distribution
    ...
    twoD.AddAlphaObj(
        f'QCD_fail_{r_fail}',     # Object name
    ...
    for rc_name, rc_opts in _rc_options.items():
        ...
        qcd_Rc = ParametricFunction(
            f'QCD_Rc_{r_pass}_{rc_name}',                    # Name of Rc function
        ...
        qcd_pass = qcd_fail.Multiply(
            f'QCD_pass_{r_pass}_{rc_name}',     # Name of the binned distributon representing QCD in VS2
            qcd_Rc          # ParametricFunction object multiplying QCD_fail
        )
        # Add it to the 2DAlphabet ledger
        twoD.AddAlphaObj(
            f'QCD_pass_{r_pass}_{rc_name}',  # Object name
...
if __name__ == "__main__":
    ...
    if args.make:
        if args.mode == "1p1":
            PFR_MC = "1.75e-6 * exp(0.0017 *(y * (5000 - 200) + 200) ) + 4.48e-4  "
            PFR_MC = "7e-6 * exp(0.0017 * (y * (5 - 0.2) + 0.2)) + 1.792e-3  "
            PFR_MC = "1.2957e-06 * exp(0.0018 * (y * (5 - 0.2) + 0.2)) + 9.4844e-04"
        elif args.mode == "2p1":
            PFR_MC = " 0.0005 "
        fr = {"SignalMC_XHY4b_MX-XMASS_MY-YMASS":f"SignalMC_XHY4b_MX-{MX}_MY-{MY}"}
        #make_2DAlphabet_workspace(name=args.wsp, fr=fr, json=f'XYH_{args.mode}_{args.r_pass}_{args.r_fail}.json', r_fail = args.r_fail, r_pass = args.r_pass, PFR_MC = PFR_MC)
        make_2DAlphabet_workspace(name=args.wsp, fr=fr, json=f'XYH_{args.r_pass}_{args.r_fail}.json', r_fail = args.r_fail, r_pass = args.r_pass, PFR_MC = PFR_MC)
