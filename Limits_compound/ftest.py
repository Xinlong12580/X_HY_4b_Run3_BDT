...
parser.add_argument('--work_dir', type=str, action='store', required=False)
parser.add_argument('--extra', type=str, action='store', required=False)
...
for subdir in subdirs:
    ...
    for f in files:
        if "GoodnessOfFit" in f and ".root" in f and "123456" not in f:
            fs.append(subdir + f)
        elif f == "card.txt":
            p = 0
            n = 0
            ...
            ps.append(p)
            ns.append(n/2)
            #ns.append(getNBins(subdir + "/../base.root" ))
print("Results: ", fs, ps, ns)
if ns[0] == ns[1]:
    n = ns[0]
...
xs = np.linspace(0, 15, 1500)
ys = [F_dist.pdf(x) for x in xs]

fig = plt.figure(figsize = (10, 6))
ax = fig.add_subplot(1, 1, 1)
ax.plot(xs, ys, color = "red")
ax.set_title(f"{args.extra} {tfs[0]} tested against {tfs[1]}")
...
fig.savefig(f"ftests/F_test_{tfs[0]}_{tfs[1]}_{args.extra}.png")
