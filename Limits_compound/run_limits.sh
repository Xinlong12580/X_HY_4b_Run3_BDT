...
verbosity=2
options=$(getopt -o "bl" --long "bfit,limits,seed:,numtoys:,fitdir:,fitdir;,strat:,verbosity:,paras:" -- "$@")
eval set -- "$options"
while true; do
    case "$1" in
        --strat)
...
IFS=';' read -ra _paras <<< "$paras"

for para in "${_paras[@]}"; do
    echo $para
    while IFS= read -r line; do
        if [[ $line == *"QCD_Rc_VR"* ]]; then
            echo 1
