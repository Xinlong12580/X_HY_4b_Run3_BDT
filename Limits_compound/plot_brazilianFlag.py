...
for MY in Limits["YSlice"]:
    ...
    if str(MY) in run2_Limits["YSlice"]:
        MXs = []
        Obses = []
        for MX in run2_Limits["YSlice"][str(MY)]:
            MXs.append(int(MX))
        MXs.sort()    
        for MX in MXs:
            Obses.append(run2_Limits["YSlice"][str(MY)][str(MX)][2])
...
for MX in Limits["XSlice"]:
    ...
    if str(MX) in run2_Limits["XSlice"]:
        print("TEST")
        MYs = []
        Obses = []
        for MY in run2_Limits["XSlice"][str(MX)]:
            MYs.append(int(MY))
        MYs.sort()    
        for MY in MYs:
            Obses.append(run2_Limits["XSlice"][str(MX)][str(MY)][2])
