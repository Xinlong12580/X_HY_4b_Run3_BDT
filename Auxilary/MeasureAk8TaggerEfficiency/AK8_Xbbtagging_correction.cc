    ...
    for (int i =0; i < nJet; i++){
        ...
        if (score_discrete == 0){
            ...
            for (const int wp : WPs){
                ...
                boost::property_tree::ptree wp_tree = SF_tree.get_child(std::to_string(wp));
                boost::property_tree::ptree flavor_tree = wp_tree.get_child(std::to_string(flavor));
                for(const auto & pt_range : flavor_tree){
                    std::string pt_key = pt_range.first;
                    size_t pt_pos = pt_key.find('_');
                    ...
                    if (pt_low <= pt && pt < pt_high){
                        ...
                        for(const auto & abseta_range : pt_tree){
                            ...
                            if (abseta_low <= abseta && abseta < abseta_high){
                                ...
                                SF_ups[wp - 1] = SF_noms[wp - 1] + abseta_range.second.get<float>("up");
                                SF_downs[wp - 1] = SF_noms[wp - 1] - abseta_range.second.get<float>("down");
                                break;
                            }
                        } 
                        break;
            ...
            if (1 - effs[0] - effs[1] - effs[2] > 0){
                SFs[0] = (1 - SF_noms[0] * effs[0] - SF_noms[1] * effs[1] - SF_noms[2] * effs[2]) / (1 - effs[0] - effs[1] - effs[2]);
                SFs[1] = (1 - SF_ups[0] * effs[0] - SF_ups[1] * effs[1] - SF_ups[2] * effs[2]) / (1 - effs[0] - effs[1] - effs[2]);
                SFs[2] = (1 - SF_downs[0] * effs[0] - SF_downs[1] * effs[1] - SF_downs[2] * effs[2]) / (1 - effs[0] - effs[1] - effs[2]);
             }
                ...
                if (pt_low <= pt && pt < pt_high){
                    ...
                    for(const auto & abseta_range : pt_tree){
                        ...
                        if (abseta_low <= abseta && abseta < abseta_high){
                            ...
                            SF[1] = SF[0] + abseta_range.second.get<float>("up");
                            SF[2] = SF[0] - abseta_range.second.get<float>("down");
                            break;
                        }
                    } 
                    break;
