...
for massX in massXs:
    for massY in massYs:
        ...
        files = [files[0] ]
        print(files)
        #files = [f"root://cmseos.fnal.gov//store/user/xinlongl/XHY4bRun3_selection_1p1_BDT/nom_tagged_selected_SKIM_skimmed_2022EE__SignalMC_XHY4b__MX-{massX}_MY-{massY}_n-10000_i-0.root"]
        ...
        rdf = rdf.Define("DeltaEta", "abs(higgs_eta - Y_eta)")
        rdf = rdf.Define("DeltaY", "abs(DeltaRapidity(higgs_pt, higgs_eta, higgs_phi, higgs_mass, Y_pt, Y_eta, Y_phi, Y_mass))")
        ...
        '''
        rdf = rdf.Filter("abs(higgs_eta) < 2.5")
        rdf = rdf.Filter("abs(Y_eta) < 2.5")
        h_deltaEtas.append(rdf.Histo1D((f"gen_MX{massX}_MY{massY}_delta_eta", f"gen_MX{massX}_MY{massY}_deta_eta", 51, 0, 5), f"DeltaEta").GetValue().Clone())
        h_deltaYs.append(rdf.Histo1D((f"gen_MX{massX}_MY{massY}_delta_Y", f"gen_MX{massX}_MY{massY}_deta_Y", 51, 0, 5), f"DeltaY").GetValue().Clone())
...
for massX in massXs:
    for massY in massYs:
        leg.AddEntry(h_deltaEtas[i], f"massX{massX}_massY{massY}")
        h_deltaEtas[i].SetLineColor(colors[i])
        h_deltaEtas[i].Scale(1/h_deltaEtas[i].Integral())
        if i == 0:
            h_deltaEtas[i].Draw("HIST")
        else:
            h_deltaEtas[i].Draw("HIST SAME")
        i += 1
leg.Draw()
c.Update()
c.Print(f"gen_compare_DeltaEta.png")
...
for massX in massXs:
    for massY in massYs:
        leg.AddEntry(h_deltaYs[i], f"massX{massX}_massY{massY}")
        h_deltaYs[i].SetLineColor(colors[i])
        h_deltaYs[i].Scale(1/h_deltaYs[i].Integral())
        if i == 0:
            h_deltaYs[i].Draw("HIST")
        else:
            h_deltaYs[i].Draw("HIST SAME")
        i += 1
leg.Draw()
c.Update()
c.Print(f"gen_compare_DeltaY.png")
