...
# Load data from file
data = np.loadtxt(f"best_scores_discrete_{args.mode}_{args.year}.txt", skiprows = 1)
...
plt.title(r"$BDTG_{threshold}(M_X, M_Y)$ from local optimization, " + f"{args.mode}, {args.year}")
...
# Save plot
plt.savefig(f"linear_best_BDT_score_discrete_{args.mode}_{args.year}.png", dpi=300)
plt.xscale("log")
plt.yscale("log")
plt.savefig(f"log_best_BDT_score_discrete_{args.mode}_{args.year}.png", dpi=300)
...
for i in range(len(labels)): 
    ...
    sc = plt.scatter(MX_vals, MY_vals, c=data[:, 2 + i], cmap="viridis", s=100, edgecolors='k', vmax=0.01)

    cbar = plt.colorbar(sc)

    plt.xlabel(r"$M_X/GeV$")
    plt.ylabel(r"$M_Y/GeV$")
    plt.title(f"{labels[i]} from local optimization, " + f"{args.mode}, {args.year}")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"linear_{labels[i]}_discrete_{args.mode}_{args.year}.png", dpi=300)
    plt.xscale("log")
    plt.yscale("log")
    plt.savefig(f"log_{labels[i]}_discrete_{args.mode}_{args.year}.png", dpi=300)
