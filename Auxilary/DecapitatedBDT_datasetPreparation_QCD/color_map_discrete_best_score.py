...
for MY in data[:, 1]:
    if MY not in MYs:
        MYs.append(MY)
for MY in MYs:
    MXs_raw_all = [ data[ind, 0] for ind in range(len(data[:,0])) if data[ind, 1] == MY and data[ind, 0] > 900 ]
    for i in range(len(labels)):
        # Create scatter plot with colormap
        plt.figure(figsize=(10, 6))
        #sc = plt.scatter(MX_vals, MY_vals, c=data[:, 2 + i], norm=LogNorm(), cmap="viridis", s=100, edgecolors='k', vmax=0.01)
        vals_raw_all = [ data[ind, i + 2] for ind in range(len(data[:,0])) if data[ind, 1] == MY and data[ind, 0] > 900 ]
        MXs_raw = [MXs_raw_all[ind] for ind in range(len(vals_raw_all)) if vals_raw_all[ind] >= 0 ]
        vals_raw = [vals_raw_all[ind] for ind in range(len(vals_raw_all)) if vals_raw_all[ind] >= 0 ]
        _pass = 0
        for val in vals_raw:
            if val > 0:
                _pass = 1
