...
public:
    ...
    float eval(std::vector<std::variant<int, float>> inputs);
...
float MVA_evaluator::eval(std::vector<std::variant<int, float>> inputs){
    for(int i = 0; i < n_vals; i ++){
        auto input = inputs.at(i);
        float input_f = std::visit([](auto x) {
            return static_cast<float>(x);
        }, input);
        std::cout<<input_f<<std::endl;     
        vals[i] = input_f;
...
void test(){
    MVA_evaluator evaluator(4, std::vector<std::string>({ "Delta_Y", "MassHiggsCandidate_regressed", "Tagger_H_discrete", "Tagger_Y_discrete"}), "dataset_1p1_2022_discrete/weights/TMVAClassification_BDTG_1p1_2022.weights.xml", 0, {} );
    float score  = evaluator.eval(std::vector<std::variant<int, float>>({ 1.8, 125., 2, 2}));
    cout<<score<<endl;
}
