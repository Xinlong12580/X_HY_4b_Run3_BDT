...
if args.mode == "1p1":
    ...
    ROOT.gInterpreter.Declare('MVA_evaluator evaluator(4, std::vector<std::string>({ "Delta_Y", "MassHiggsCandidate_regressed", "Tagger_H_decapitated", "Tagger_Y_decapitated"}), "dataset_MX' + args.mx + "_MY" + args.my + "_"  + args.mode + "_" + args.year +'_discrete/weights/TMVAClassification_BDTG_' + args.mode + "_" + args.year + '.weights.xml", 0, {} );')
elif args.mode == "2p1":
    ...
    ROOT.gInterpreter.Declare('MVA_evaluator evaluator(4, std::vector<std::string>({ "MassHiggsCandidate_regressed", "Tagger_H_decapitated", "Tagger_b_Y0_decapitated", "Tagger_b_Y1_decapitated"}), "dataset_MX' + args.mx + "_MY" + args.my + "_" + args.mode + "_" + args.year  +'_discrete/weights/TMVAClassification_BDTG_' + args.mode + "_" + args.year  + '.weights.xml", 0, {} );')
...
for f in fs:
    ...
    try:
        if args.mode == "1p1": 
            rdf = rdf.Define("BDTG", "evaluator.eval(std::vector<float>( {Delta_Y, MassHiggsCandidate_regressed, float(Tagger_H_decapitated), float(Tagger_Y_decapitated)}))")
        elif args.mode == "2p1":
            #rdf = rdf.Define("BDTG", "evaluator.eval(std::vector<float>({ MassHiggsCandidate, PNet_H, PNet_Y0, PNet_Y1 }))")
            rdf = rdf.Define("BDTG", "evaluator.eval(std::vector<std::variant<int, float>>({ MassHiggsCandidate_regressed, float(Tagger_H_decapitated), float(Tagger_b_Y0_decapitated), float(Tagger_b_Y1_decapitated) }))")
