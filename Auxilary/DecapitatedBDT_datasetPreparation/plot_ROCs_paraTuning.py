...
with open(f"ROCs_MX{args.mx}_MY{args.my}_{args.mode}_{args.year}.pkl", "rb") as f:
    ROCs = pickle.load(f)
    AUCs = pickle.load(f)
...
ax.set_title(f"ROCs for parameter tuning, MX {args.mx} MY {args.my}, {args.mode} channel, year {args.year}")
ax.set_xlabel("Signal Efficiency")
ax.set_ylabel("Background Rejection Rate")
#ax.legend()
fig.savefig(f"ROCs_MX{args.mx}_MY{args.my}_{args.mode}_{args.year}.png")
print(max_AUC, max_config)
with open(f"max_config_MX{args.mx}_MY{args.my}_{args.mode}_{args.year}.txt", "w") as f:
