...
with open("handmade_2p1_discrete.txt", "r") as f:
    lines = f.readlines()
    paras = []
    for i in range(len(lines)):
        ...
        print(para)
        paras.append(para)
        #f2.SetParameters(f"[{i}]",para)
    f2.SetParameters(paras[0], paras[1], paras[2])
    
with open(f"handmade_DDT_map_para_discrete_{args.mode}.txt", "w") as _f: 
    for i in range(f2.GetNpar()):
        print(f2.GetParameter(i))
        _f.write(f"{f2.GetParameter(i)}\n")
    

with open(f"handmade_score_discrete_{args.mode}.txt", "w") as _f:
