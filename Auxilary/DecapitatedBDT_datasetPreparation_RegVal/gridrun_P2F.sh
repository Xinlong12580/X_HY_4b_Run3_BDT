#!/bin/bash
root_dir=$(pwd)
base_file=$(basename $6)
OUTTXT=MX-$2_MY-$4_"$base_file"_output.log
OUTTXT="${OUTTXT//\//_}"
echo "OUTTXT" $OUTTXT
...
# MAIN FUNCTION
python optimize_P2F.py $* 2>&1 | tee -a $root_dir/$OUTTXT
status=${PIPESTATUS[0]}
if [ $status -eq 0 ]; then
    # move all snapshots to the EOS (there will only be one)
    xrdcp -f *.root root://cmseos.fnal.gov//store/user/xinlong/XHY4bRun3_check_match/
else
    mv $root_dir/$OUTTXT $root_dir/FAILED_$OUTTXT
    OUTTXT=FAILED_$OUTTXT
fi
#ls | tee -a $root_dir/$OUTTXT
xrdcp -f $root_dir/$OUTTXT root://cmseos.fnal.gov//store/user/xinlong/XHY4bRun3_check_match/
