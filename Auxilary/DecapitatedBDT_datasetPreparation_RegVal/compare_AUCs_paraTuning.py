...
f = ROOT.TFile.Open(f"TMVAC_optimization_MX{args.mx}_MY{args.my}_{args.mode}_{args.year}_discrete.root", "READ")
#f = ROOT.TFile.Open(f"TMVAC_optimization_{args.mode}_{args.year}_discrete.root", "READ")
BDT_dir = f.Get(f"dataset_optimization_MX{args.mx}_MY{args.my}_{args.mode}_{args.year}_discrete/Method_BDT")
...
print(AUCs)
with open(f"AUCs_MX{args.mx}_MY{args.my}_{args.mode}_{args.year}.txt", "w") as _f:
    for config_name in AUCs:
        _f.write(f"{config_name} {AUCs[config_name]}\n")
with open(f"ROCs_MX{args.mx}_MY{args.my}_{args.mode}_{args.year}.pkl", "wb") as _f:
        pickle.dump(ROCs, _f)
        pickle.dump(AUCs, _f)
