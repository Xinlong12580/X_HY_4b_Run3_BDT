...
for f in fs:
    ...
    try:
        if args.mode == "1p1": 
            ...
            rdf = rdf.Define("BDTG", "evaluator.eval(std::vector<std::variant<int, float>>({ MassHiggsCandidate, float(Tagger_H_decapitated), float(Tagger_b_Y0_decapitated), float(Tagger_b_Y1_decapitated) }))")
