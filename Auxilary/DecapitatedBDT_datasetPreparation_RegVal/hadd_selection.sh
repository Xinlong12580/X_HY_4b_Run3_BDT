...
year=$2
#rm datasets/*root
input_dir=/store/user/xinlong/XHY4bRun3_selection_combined_BDT
output_dir="./datasets/"
eosmkdir -p "$output_dir"
echo "TEST!"
files=$( eosls $input_dir | grep RegVal | grep "$year"_ | grep "nom" )
prefix=$eosprefix$input_dir/
declare -A classified_files
declare -A classified_file_idxs
for file in ${files[@]}; do
    if [[ $file == *"Template"* || $file != *"$year"_* || $file != *"nom"* || $file == *"Signal"*  ]]; then
        continue
    fi
    file_base="${file%%_n-*}"
