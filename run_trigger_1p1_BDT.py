...
columns = ["n.*", "HLT_.*", "TFlavor.*", "Tagger_.*", "MassHiggs.*", "idx.*", "gen_.*", ".*_matched", "Delta_Eta", "Delta_Y", ".*Weight", ".*weight", "MY", "MX", "leadingFatJetPt","leadingFatJetPhi","leadingFatJetEta", "leadingFatJetMsoftdrop", "MassLeadingTwoFatJets", "MassHiggsCandidate", "PtHiggsCandidate", "EtaHiggsCandidate", "PhiHiggsCandidate", "MassYCandidate", "PtYCandidate", "EtaYCandidate", "PhiYCandidate", "MJJ", "MJY", "PNet_H", "PNet_Y", "weight.*",  "FatJet_pt_JER__up", "PileUp_Corr__nom", "PileUp_Corr__up", "PileUp_Corr__down", "Pileup_nTrueInt"]

for Reg in ["Signal"]:
#Running selection
    ana = XHY4b_Analyzer(args.dataset, args.year, args.n_files, args.i_job)
    ana.selection_1p1_BDT(args.JME_syst, Reg, triggerType = "Reference")
    #ana.trainer_preprosessing_1p1()
    #Saving snapshot and cutflow
    file_basename = os.path.basename(args.dataset).removesuffix(".txt")
    ana.output = "Reg" + Reg[0:3] + "_" + args.JME_syst + "_1p1_trigger_tagged_selected_" + file_basename + f"_n-{args.n_files}_i-{args.i_job}.root"
